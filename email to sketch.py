...
img = filedialog.askopenfilename(initialdir = os.getcwd(), title='Select File', filetypes=(('JPEG','*.jpg'),('PNG','*.png'),('All files','*.*')))

# ss = imageio.imread(img)    ## to read the given image

def rgb2gray(rgb):
    return np.dot(rgb[...,:3],[0.2989,0.5870,0.1140]) ## it is 2 - dimensional array formula to convert image to gray scale

def dodge(front,back):
    final_sketch = front*255/(255-back)
    ## if image is greater 255 which i don't think is possible but still if it is there will convert it to 255
...
cv2.imwrite('pic2_gray.jpg',r)

# Imports PIL module
from PIL import Image

# open method used to open different extension image file
im = Image.open(r"pic2_gray.jpg")
